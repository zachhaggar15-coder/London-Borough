...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const formatted = `${money(salary, currency)}`;
  const region = content.copy.regionLabel;
  const title = `Where to live in ${region} on a ${formatted} salary`;
  const description = `On ${formatted} you take home about ${money(data.takeHomeMonthly, currency)} a month, giving a rent budget near ${money(data.budget35, currency)}. ${data.comfortable.length} of ${content.areas.length} ${region} areas fit inside it.`;
...
export default async function CitySalaryPage({ params }: Props) {
  ...
  const verdict =
    comfortable.length === 0
      ? `On ${formatted} no one-bed in ${region} fits inside the 35% guideline. The cheapest anywhere is ${cheapestOneBed.area.name} at ${money(cheapestOneBed.rentGbp, currency)}, which would take ${Math.round(cheapestOneBed.shareOfTakeHome * 100)}% of your take-home. Sharing is the realistic route at this salary, and it is a perfectly normal one here.`
      : comfortable.length === 1
        ? `On ${formatted} exactly one area fits inside the guideline for a one-bed: ${comfortable[0].area.name}, at ${money(comfortable[0].rentGbp, currency)} a month. A room in a share opens up a great deal more.`
        : comfortable.length >= total * 0.85
          ...
          ? `On ${formatted} rent stops being the constraint: ${comfortable.length} of the ${total} areas fit a one-bed inside the guideline, ${topOfRange.area.name} at ${money(topOfRange.rentGbp, currency)} among them. The question becomes commute and character rather than affordability — this is the salary at which the centre, ${secondFromTop.area.name} and the rest of the inner ring are genuinely open to you.`
          : `On ${formatted}, ${comfortable.length} of the ${total} areas covered here fit a one-bed inside the 35% guideline, from ${comfortable[0].area.name} at ${money(comfortable[0].rentGbp, currency)} upwards.`;
    ...
    mainEntity: [
      {
        ...
        acceptedAnswer: {
          "@type": "Answer",
          text: `About ${money(takeHomeMonthly, currency)} a month after income tax and National Insurance, before pension contributions or student loan repayments. That gives a rent budget of roughly ${money(budget33, currency)} at 33% or ${money(budget35, currency)} at 35%.`,
        },
      },
      {
        "@type": "Question",
    ...
    <>
      ...
      <PageShell>
        ...
        <Section
          ...
          lead={
            roomShareWithinBudget.length > 0
              ? `${roomShareWithinBudget.length} areas have a typical room inside your budget.${copy.roomIncludesNote ? ` ${copy.roomIncludesNote}` : ""}`
        ...
        <DataNote>
          ...
          further. Rents are reviewed estimates anchored on{" "}
          {input.rent.baselineLabel} by {input.councilNoun.singular} and
          exclude the recurring costs set out in{" "}
          {copy.rentExtrasNote ?? "the local costs table"} — see the{" "}
          <Link
