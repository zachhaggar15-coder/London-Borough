...
/**
 * Only the cities that call their authorities "arrondissements" build here.
 * The others answer on the sibling segment, and this one 404s for them.
 */
export async function generateStaticParams() {
  return citiesPublishing("arrondissements").filter(
    (city) => CITIES[city].councilSegment === "arrondissements",
  ).map((city) => ({ city }));
}
...
export default async function Page({ params }: Props) {
  const { city } = await params;
  if (!isContentCityId(city) || CITIES[city].councilSegment !== "arrondissements") {
    notFound();
  }
