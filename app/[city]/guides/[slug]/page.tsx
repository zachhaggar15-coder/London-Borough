...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const guide = content.getGuide(slug);
  if (!guide) return {};

  return {
    title: guide.metaTitle,
    description: guide.metaDescription,
    alternates: { canonical: content.url(`/guides/${slug}`) },
    openGraph: {
      title: guide.metaTitle,
      description: guide.metaDescription,
      url: content.url(`/guides/${slug}`),
      type: "article",
...
export default async function CityGuidePage({ params }: Props) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Guides", item: content.url("/guides") },
      {
        "@type": "ListItem",
        position: 3,
        name: guide.h1,
        item: content.url(`/guides/${slug}`),
      },
    ],
  };
    ...
    <>
      ...
      <PageShell>
        <CityBreadcrumbs
          content={content}
          trail={[
            { label: "Guides", href: content.path("/guides") },
        ...
        <article>
          ...
          {guide.sections.map((section) => (
            ...
            >
              ...
              {section.dataBlock && (
                <GuideDataBlock block={section.dataBlock} content={content} />
              )}
            </section>
          ))}
        ...
        <section className="mt-12 border-t border-slate-800 pt-8">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            {others.map((other) => (
              <Link
                key={other.slug}
                href={content.path(`/guides/${other.slug}`)}
