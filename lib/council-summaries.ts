...
/**
 * Local-authority rollups for the map's council search.
 *
 * Produces the same BoroughSummary shape the London map already consumes,
 * so the search box and the boundary outline layer work unchanged.
 *
 * The one behavioural difference from London: London throws if a borough
 * has no scored areas, because all 32 are covered. Coverage outside
 * London is uneven by design — Wigan has three areas and the city of
 * Manchester sixteen — so an authority with nothing scored is omitted
 * rather than treated as a data error.
 */
export function councilSummaries(
  ...
  return councils
    .flatMap((council) => {
      const scored = scoredNeighbourhoods
        .filter((item) => item.neighbourhood.borough === council)
        .sort(compareMatches);
